...
class Membre extends Gvv_Controller {
    ...
    /**
     * Anonymise un membre en remplaçant les données personnelles par des valeurs aléatoires
     * et propage les changements vers les comptes 411 associés
     * 
     * @param string $mlogin Login du membre à anonymiser
     */
    public function anonymize($mlogin) {
        ...
        $membre = $this->gvv_model->get_by_id('mlogin', $mlogin);
        if (!$membre) {
            $data['msg'] = "Le membre $mlogin n'existe pas";
            load_last_view('error', $data);
            return;
        }
        ...
        $villes = [
            ...
            'Aix-en-Provence', 'Clermont-Ferrand', 'Brest', 'Limoges', 'Tours', 'Amiens', 'Perpignan', 'Metz', 'Besançon', 'Orléans',
            'Mulhouse', 'Rouen', 'Caen', 'Nancy', 'Argenteuil', 'Montreuil', 'Roubaix', 'Tourcoing', 'Nanterre', 'Avignon',
            'Créteil', 'Poitiers', 'Versailles', 'Courbevoie', 'Vitry-sur-Seine', 'Pau', 'La Rochelle', 'Calais', 'Cannes', 'Antibes',
            'Dunkerque', 'Bourges', 'Quimper', 'Valence', 'Troyes', 'Lorient', 'Montauban', 'Cholet', 'Charleville-Mézières', 'Laval',
            'Boulogne-Billancourt', 'Saint-Denis', 'Aubervilliers', 'Champigny-sur-Marne', 'Saint-Maur-des-Fossés', 'Asnières-sur-Seine',
            'Rueil-Malmaison', 'Colombes', 'Aulnay-sous-Bois', 'Versailles', 'Tourcoing', 'Nîmes', 'Créteil', 'Pau',
            'Bayonne', 'Vannes', 'Châlons-en-Champagne', 'Évreux', 'Chartres', 'Blois', 'Châteauroux', 'Nevers'
        ];
        $adresses = ['12 rue de la Paix', '5 avenue Victor Hugo', '23 boulevard Haussmann', '8 place de la République', '15 rue Molière',
                    '7 avenue des Champs', '19 rue Pasteur', '3 place Bellecour', '11 rue de Rivoli', '6 avenue Foch'];
        ...
        $nouvelle_ville = $villes[array_rand($villes)];
        $nouvelle_adresse = $adresses[array_rand($adresses)];
        ...
        // Générer des numéros de téléphone français aléatoires
        $prefixes = ['01', '02', '03', '04', '05', '06', '07', '09'];
        $prefix_fixe = $prefixes[array_rand($prefixes)];
        $prefix_mobile = $prefixes[array_rand($prefixes)];
        $nouveau_tel_fixe = $prefix_fixe . sprintf('%02d%02d%02d%02d', rand(10, 99), rand(10, 99), rand(10, 99), rand(10, 99));
        $nouveau_tel_mobile = $prefix_mobile . sprintf('%02d%02d%02d%02d', rand(10, 99), rand(10, 99), rand(10, 99), rand(10, 99));
        
        // Générer un code postal cohérent avec la ville
        $nouveau_cp = rand(10000, 99999);
        
        // Mettre à jour le membre
        $data_update_membre = array(
            'mlogin' => $mlogin,
            'mnom' => $nouveau_nom,
            'mprenom' => $nouveau_prenom,
            'memail' => $nouvel_email,
            'memailparent' => '', // Vider l'email des parents
            'madresse' => $nouvelle_adresse,
            'ville' => $nouvelle_ville,
            'cp' => $nouveau_cp,
            'mtelf' => $nouveau_tel_fixe,
            'mtelm' => $nouveau_tel_mobile,
            'mlieun' => $nouvelle_ville, // Lieu de naissance = ville actuelle
            'comment' => '', // Vider les commentaires
            'profession' => '', // Vider la profession
            'place_of_birth' => $nouvelle_ville
        );
        
        $this->gvv_model->update('mlogin', $data_update_membre);
        
        // Mettre à jour les comptes 411 associés si le membre a un compte
        if ($membre['compte'] && $membre['compte'] > 0) {
            $this->load->model('comptes_model');
            
            // Vérifier que c'est bien un compte 411
            $compte = $this->comptes_model->get_by_id('id', $membre['compte']);
            if ($compte && strpos($compte['codec'], '411') === 0) {
                $nouveau_nom_compte = $nouveau_nom . ' ' . $nouveau_prenom;
                $nouvelle_desc_compte = 'Compte client ' . $nouveau_nom . ' ' . $nouveau_prenom;
                
                $data_update_compte = array(
                    'id' => $membre['compte'],
                    'nom' => $nouveau_nom_compte,
                    'desc' => $nouvelle_desc_compte
                );
        ...
        // IMPORTANT: Mettre à jour TOUS les comptes 411 qui référencent ce membre via comptes.pilote
        $this->load->model('comptes_model');
        
        // Mettre à jour tous les comptes 411 où pilote = mlogin
        $this->db->where('codec LIKE', '411%');
        $this->db->where('pilote', $mlogin);
        $comptes_pilote = $this->db->get('comptes')->result_array();
        
        foreach ($comptes_pilote as $compte_pilote) {
            $nouveau_nom_compte = $nouveau_nom . ' ' . $nouveau_prenom;
            $nouvelle_desc_compte = 'Compte client ' . $nouveau_nom . ' ' . $nouveau_prenom;
        ...
        $data['text'] = "Le membre $mlogin a été anonymisé avec succès." . 
                       ($membre['compte'] && $membre['compte'] > 0 ? " Le compte associé a également été mis à jour." : "");
        load_last_view('message', $data);
    }
    ...
    public function anonymize_all() {
        ...
        foreach ($results as $row) {
            ...
            // Récupérer les données du membre pour le compte
            $membre = $this->gvv_model->get_by_id('mlogin', $mlogin);
        ...
        // Message de succès
        $data['title'] = "Anonymisation réussie";
        $data['text'] = "$count_membres membres ont été anonymisés avec succès." . 
                       ($count_comptes > 0 ? " $count_comptes comptes associés ont également été mis à jour." : "");
        load_last_view('message', $data);
    }
}
