...
class FPDF {
    ...
    *******************************************************************************/
    function __construct($orientation = 'P', $unit = 'mm', $size = 'A4') {
        ...
        $this->Error('Incorrect orientation: ' .
        $orientation);
    ...
    function SetDisplayMode($zoom, $layout = 'default') {
        ...
        $this->Error('Incorrect zoom display mode: ' .
        $zoom);
        if ($layout == 'single' || $layout == 'continuous' || $layout == 'two' || $layout == 'default')
        $this->LayoutMode = $layout;
        else
        $this->Error('Incorrect layout display mode: ' .
        $layout);
    }
    ...
    function Cell($w, $h = 0, $txt = '', $border = 0, $ln = 0, $align = '', $fill = false, $link = '') {
        ...
        if ($txt !== '') {
            ...
            if ($this->ColorFlag)
            $s .= 'q ' . $this->TextColor . ' ';
            // patch to support utf8
            // $txt2 = str_replace(')', '\\)', str_replace('(', '\\(', str_replace('\\', '\\\\', $txt)));
            // replace
            // $s .= sprintf('BT %.2F %.2F Td (%s) Tj ET', ($this->x + $dx) * $k, ($this->h - ($this->y + .5 * $h +.3 * $this->FontSize)) * $k, $txt2);
            // by
            $s .= sprintf('BT %.2F %.2F Td (%s) Tj ET', ($this->x + $dx) * $k, ($this->h - ($this->y + .5 * $h +.3 * $this->FontSize)) * $k,
                     $this->_escape($txt));
            if ($this->underline)
    ...
    *******************************************************************************/
    ...
    function _parsepng($file) {
        ...
        $f = fopen($file, 'rb');
        if (!$f)
        $this->Error('Can\'t open image file: ' .
        $file);
    ...
    function _putfonts() {
        ...
        foreach ($this->fonts as $k => $font) {
            ...
            elseif ($type == 'Type1' || $type == 'TrueType') {
                ...
                if (!method_exists($this, $mtd))
                $this->Error('Unsupported font type: ' .
                $type);
                $this-> $mtd ($font);
            }
        }
    ...
    function _putinfo() {
        ...
        if (!empty ($this->title))
        $this->_out('/Title ' .
        $this->_textstring($this->title));
        if (!empty ($this->subject))
        $this->_out('/Subject ' .
        $this->_textstring($this->subject));
        if (!empty ($this->author))
        $this->_out('/Author ' .
        $this->_textstring($this->author));
        if (!empty ($this->keywords))
        $this->_out('/Keywords ' .
        $this->_textstring($this->keywords));
        if (!empty ($this->creator))
        $this->_out('/Creator ' .
        $this->_textstring($this->creator));
    ...
    function _putcatalog() {
        ...
        elseif (!is_string($this->ZoomMode)) $this->_out('/OpenAction [3 0 R /XYZ null null ' .
