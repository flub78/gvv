 ...
 *
 */
class Migration_Events_Annual extends CI_Migration {
	...
	{	
		...
		$sqls = array(
				"ALTER TABLE `events_types` ADD `annual` TINYINT( 1 ) COMMENT 'Evénement annuel'",
		);

		$errors += $this->run_queries($sqls); 
		gvv_info("Migration database up to 12, errors=$errors");
	...
	public function down()
	{
		...
		// nettoyage de champs inutiles
		$sqls = array(
				"ALTER TABLE `events_types` DROP `annual`",
		);

		$errors += $this->run_queries($sqls); 
		gvv_info("Migration database down to 11, errors=$errors");
