...
class Email_lists_model extends CI_Model
{
    ...
    public function get_users_by_role_and_section($types_roles_id, $section_id, $active_member = 'active', $require_cotisation = true)
    {
        ...
        if ($active_member === 'active') {
            // Active = has 'user' role in the same section as the criteria.
            ...
            // Inactive = does NOT have 'user' role in the criteria section.
            $section_id_safe = ($section_id !== NULL) ? (int)$section_id : NULL;
            if ($section_id_safe !== NULL) {
                $this->db->where("u.id NOT IN (SELECT urps_i.user_id FROM user_roles_per_section urps_i JOIN types_roles tr_i ON tr_i.id = urps_i.types_roles_id WHERE tr_i.nom = 'user' AND urps_i.section_id = $section_id_safe AND urps_i.revoked_at IS NULL)", NULL, FALSE);
            } else {
                $this->db->where("u.id NOT IN (SELECT urps_i.user_id FROM user_roles_per_section urps_i JOIN types_roles tr_i ON tr_i.id = urps_i.types_roles_id WHERE tr_i.nom = 'user' AND urps_i.revoked_at IS NULL)", NULL, FALSE);
            }
        }
        // 'all' - no filter
