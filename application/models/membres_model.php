...
class Membres_model extends Common_Model {
    ...
    public function select_page($nb = 1000, $debut = 0, $selection = array()) {
        $select = $this->db->select('mlogin, mprenom, trigramme, mnom, madresse, cp, ville, mtelf, mtelm, memail, mdaten, m25ans, msexe, actif, exemption_solde, categorie, photo, place_of_birth, inscription_date, validation_date')->from($this->table)->order_by('mnom, mprenom')->where($selection)->
        // ->limit($nb, $debut)
        get()->result_array();
    ...
    /**
     * Retourne un sélecteur des instructeurs inscrits dans une section spécifique
     * Un instructeur est considéré inscrit dans une section s'il:
     * - Possède un compte 411 actif dans cette section
     * - A le rôle "instructeur" (types_roles_id = 11) pour cette section
     *
     * @param int $section_id L'ID de la section (0 = section active)
     * @param bool $only_actif true pour ne retourner que les membres actifs, false pour tous
     * @return array Sélecteur avec les instructeurs de la section au format [mlogin => image]
     */
    public function inst_selector($section_id = 0, $only_actif = true) {
        // Détermine la section à utiliser
        if ($section_id == 0) {
            $section_id = $this->section_id();
        }
        ...
        // Sélectionne les membres qui ont un compte 411 actif dans la section
        // et qui ont le rôle instructeur pour cette section
        $this->db->distinct();
        $this->db->select('membres.mlogin');
        $this->db->from('membres');
        $this->db->join('comptes', 'comptes.pilote = membres.mlogin', 'inner');
    ...
    public function treuillard_selector($section_id = 0, $only_actif = true) {
        if ($section_id == 0) {
            $section_id = $this->section_id();
        }
     ...
     * @param bool $only_actif Only active members
     * @return array Selector array [mlogin => 'nom prenom']
     */
    public function get_selector($section_id = 0, $only_actif = true) {
        // Détermine la section à utiliser
        if ($section_id == 0) {
            $section_id = $this->section_id();
        }
     ...
     *
     * @param int $section_id Section ID (0 = current section)
     * @param bool $only_actif Only active instructors
     * @return array Selector array [mlogin => 'nom prenom']
     */
