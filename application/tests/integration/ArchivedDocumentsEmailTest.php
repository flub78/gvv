...
class ArchivedDocumentsEmailTest extends TestCase
{
    ...
    /**
     * Non-regression: the email modal template must not include direct
     * visualization/download URLs in the generated message body.
     */
    public function testEmailTemplateDoesNotEmbedVisualizationUrlsInBody()
    {
        $view_file = APPPATH . 'views/archived_documents/bs_documentsListView.php';
        $this->assertFileExists($view_file);

        $content = file_get_contents($view_file);
        $this->assertNotFalse($content);

        // Ensure the email body builder function exists.
        $this->assertStringContainsString('function buildEmailBody(subject)', $content);

        // No archived_documents direct links should be embedded in message template.
        $this->assertFalse((bool) preg_match('/buildEmailBody\\(subject\\)[\\s\\S]*archived_documents\\/(view|preview|download)\\//', $content),
            'Le corps d\'email ne doit pas contenir de lien direct vers view/preview/download');

        // No generic HTTP links should be generated by default in the body template.
        $this->assertFalse((bool) preg_match('/buildEmailBody\\(subject\\)[\\s\\S]*https?:\\/\\//', $content),
            'Le corps d\'email ne doit pas inclure d\'URL HTTP/HTTPS par défaut');
    }

    /**
     * Non-regression: direct endpoints must keep owner/admin authorization checks.
     */
    public function testDirectEndpointsKeepAuthorizationGuards()
    {
        $controller_file = APPPATH . 'controllers/archived_documents.php';
        $this->assertFileExists($controller_file);

        $content = file_get_contents($controller_file);
        $this->assertNotFalse($content);

        // Guard used by view/download/preview: only admin or document owner.
        $owner_or_admin_guard = "if (!\$this->_is_admin() && \$doc['pilot_login'] !== \$this->dx_auth->get_username())";
        $this->assertGreaterThanOrEqual(3, substr_count($content, $owner_or_admin_guard),
            'Les endpoints directs view/download/preview doivent garder le garde owner/admin');

        // On denial, these endpoints must redirect to my_documents.
        $this->assertGreaterThanOrEqual(3, substr_count($content, "redirect('archived_documents/my_documents');"),
            'Les accès non autorisés doivent rediriger vers my_documents');

        // Private file check must still be enforced on direct file endpoints.
        $this->assertStringContainsString('if (!$this->_can_access_private_file($is_private, $doc[\'pilot_login\'])) {', $content,
            'Le contrôle d\'accès aux documents privés doit rester en place');
    }
}
