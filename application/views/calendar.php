...
$this->load->view('header', array('new_layout' => true));
...
e_div_open(array('id' => "body3", 'class' => "ui-widget-content"));
...
$table = array(
		...
		array(label('welcome_date', array('for' => 'date_ajout')),
				input(array(
				'type' => "text",
				'name' => "date_ajout",
				'id' =>"date_ajout",
		...
		array(label("gvv_membres_field_mlogin", array('for' => 'mlogin')),
				$pilot),
		array(label('welcome_intent', array('for' => 'role')),
			  form_dropdown("role", $this->lang->line("welcome_options"), "", 'id="role"')
		),
		array(label('welcome_comment', array('for' => 'commentaire')),
				input(array(
				'type' => "text",
				'name' => "commentaire",
				'id' =>"commentaire",
...
if ($club == 'accabs') {
	echo p('<div class="error">Les paiements par chèque doivent être remis exclusivement à Frédéric Peignot,</div>');
...
e_html_script(array('type' => "text/javascript", 'src' => js_url('calendar')));
?>
