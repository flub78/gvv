...
<div class="container mt-4 mb-5">
    <?php
        ...
        $submission = isset($submission) ? $submission : array('id' => 0);
    ...
    <div class="card shadow-sm">
        <div class="card-body <?= html_escape($scope_class) ?>">
            ...
            <form method="post" enctype="multipart/form-data"
                  action="<?= site_url('forms_admin/submission_edit_submit/' . (int) $form['id'] . '/' . (int) $submission['id']) ?>">
    ...
    <div class="mt-3">
        <a class="btn btn-outline-secondary btn-sm" href="<?= site_url('forms_admin/submission/' . (int) $form['id'] . '/' . (int) $submission['id']) ?>">
            <?= $this->lang->line('forms_button_cancel') ?>
        </a>
    </div>
</div>
