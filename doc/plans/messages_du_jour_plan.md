...
- [x] 3. Ecrire et tester les migrations
  ...
  - Squash (revue PR #83, finding #7) : les colonnes d'acteur (`created_by`/`updated_by` sur les 5 tables, `motd_replies.author_login`, `motd_user_message_state.user_login`, `motd_user_prefs.user_login`) avaient d'abord ete creees avec une FK stricte vers `membres`, puis relachees par deux migrations correctives ecrites en cours de developpement (`144_motd_relax_actor_fk.php`, `145_motd_relax_actor_user_login_fk.php` — voir etapes 6 et 8 ci-dessous pour le contexte des bugs qui les ont motivees). Comme rien n'avait ete deploye en production, ces 3 migrations ont ete fusionnees dans `143_create_motd_tables.php` avant merge (schema final identique, sans les FK qui etaient de toute facon supprimees juste apres) ; `144_motd_relax_actor_fk.php` et `145_motd_relax_actor_user_login_fk.php` sont supprimees et `migration_version` reste a 143.
...
- [x] 5. Configurer metadonnees et formulaires admin
  ...
  - Bug decouvert en testant (curl, utilisateur `testadmin`) : les comptes admin sans ligne `membres` (ex. `testadmin`) faisaient echouer la FK `created_by`/`updated_by`/`author_login` -> `membres(mlogin)`. Corrige a l'origine par une migration correctrice dediee, depuis fusionnee dans `143_create_motd_tables.php` (etape 3, squash post-revue) : ces colonnes sont de simple audit sans FK (cf. design doc). Test de regression ajoute dans `MotdModelTest.php` (19 tests, verts). Suite complete: 714 tests, aucune regression.
...
- [x] 6. Implementer les controleurs d'administration
  ...
  - Deux bugs decouverts et corriges en ecrivant le smoke test Playwright (`playwright/tests/motd-admin-smoke.spec.js`):
    - Le champ `level` (optionnel en base) peut arriver dans `form2database()` sans valeur soumise (client non-navigateur, ou requete brute) : `input->post()` renvoie alors `FALSE` (pas `''`) -> troncature MySQL sur l'ENUM. Corrige dans `Motd::form2database()` (`empty()` plutot que comparaison stricte a `''`). Note (revue PR #83) : la vue `bs_formView.php` presectionne "Info" par defaut en creation (`$level = isset($level) ? $level : 'info'`), donc ce cas ne se produit pas via le formulaire admin reel — voir mise a jour du PRD EF1.
...
- [x] 8. Implementer les actions utilisateur sur messages
  ...
  - Bug decouvert et corrige en testant les actions utilisateur : `motd_user_message_state.user_login` et `motd_user_prefs.user_login` avaient encore une FK vers `membres(mlogin)`. Comme ces colonnes stockent l'utilisateur connecte qui agit (pas necessairement un membre du club — un club-admin sans ligne `membres`, ex. `testadmin`, doit pouvoir masquer/prendre connaissance), toute action de ce type declenchait une violation de contrainte (erreur 1452). Corrige a l'origine par une migration correctrice dediee, depuis fusionnee dans `143_create_motd_tables.php` (etape 3, squash post-revue). Note dans `doc/design_notes/messages_du_jour_design.md` et diagramme `messages_du_jour_er.puml`/`.png` a jour en consequence.
