...
- [x] 9. Implementer la page "Tous mes messages"
  - Actions:
  ...
  - Resultat: Nouvelle action `Motd::mine()` (accessible a tout utilisateur connecte, pas de restriction club-admin), qui appelle `active_messages_for_user($username, $sort_by, exclude_hidden=FALSE)` - contrairement au dashboard, cette page reste volontairement exhaustive et affiche aussi les messages que l'utilisateur a deja masques individuellement, puisque son role est justement de permettre de les retrouver. La politique "pas d'affichage des messages expires" (decidee a l'etape 1) est deja garantie par `active_messages_for_user()`, qui filtre sur `start_date <= now <= end_date` : aucun filtre supplementaire necessaire. Nouvelle vue `application/views/motd/bs_my_messages.php` (page complete, header/menu/banner + accordeon ou message vide). L'accordeon Bootstrap (rendu, actions masquer/pris connaissance/repondre, reponses imbriquees) a ete extrait de `bs_dashboard.php` vers un partial partage `application/views/motd/_message_accordion.php`, evitant la duplication entre les deux vues et garantissant un rendu et des actions strictement identiques. Lien de navigation permanent `#motdMineLink` ajoute sur le dashboard, en dehors du bloc conditionnel de la section repliable, pour rester accessible meme quand aucun message actif non masque ne subsiste (et donc que la section repliable elle-meme n'est pas rendue).
  - Verifie : nouveau smoke test Playwright `playwright/tests/motd-my-messages-smoke.spec.js` (3 tests : lien toujours present sur le dashboard meme section absente, page listant un message masque avec sa reponse et jamais un message expire, navigation dashboard -> page dediee), utilisant un compte pilote dedie (`obelix`) pour eviter toute collision d'etat avec les autres suites MOTD. Les 4 suites Playwright MOTD (`motd-admin-smoke.spec.js`, `motd-dashboard-smoke.spec.js`, `motd-user-actions-smoke.spec.js`, `motd-my-messages-smoke.spec.js`, 15 tests au total) executees ensemble : toutes vertes, aucune collision (y compris avec les messages cible "tous les utilisateurs" crees par les autres suites, visibles par tout compte de test - geree par un masquage global auto-cicatrisant identique a celui de l'etape 8). Suite complete `phpunit_mysql.xml` : 715 tests, 18 skips preexistants sans rapport, aucune regression.
