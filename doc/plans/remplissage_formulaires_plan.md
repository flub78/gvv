...
- [x] Reconnexion des formulaires publics existants : les 6 formulaires ayant `instructor` en `required_params` (`attestation_de_test_au_sol`, `attestation_de_test_en_vol`, `declaration_debut_formation_ulm`, `test_en_vol_spl`, `attestation_provisoire_spl`, `attestation_de_formation_ulm`) avaient leur widget signature instructeur câblé sur `instructor.event.<qualif>.signature` (`events.signature_path`, colonne jamais renseignée par aucun écran) ou sans `data-gvv-source` du tout — donc jamais pré-rempli, indépendamment de la garde ci-dessus. Corrigé vers `instructor.signature` via un contrôleur one-shot dédié, rejouable : `Oneshot::signature_instructeur_wiring()` (`application/controllers/oneshot.php`, menu `/oneshot`), qui corrige à la fois le fichier `uploads/formulaires/<code>/pageNN.html` (prioritaire au runtime, cf. `Forms_file_storage::read_page()`) et la copie `form_pages.content_html` en base, avec détection d'idempotence (« déjà à jour ») pour pouvoir être rejoué sans risque, y compris sur une autre instance de production. Exécuté sur gvv.net et vérifié (fichiers + BDD passés à « corrigé », rejeu confirmé idempotent, rendu de `attestation-de-test-en-vol` vérifié sans régression).
- [x] Constat annexe (hors périmètre signature, découvert en marge) : le champ « Numéro d'identification de l'ULM » de `attestation_de_test_au_sol` et `attestation_de_test_en_vol` n'avait lui non plus aucun `data-gvv-source`, alors que `machine.numero_identification` (`machinesa.numero_identification`, migration 173, `_resolve_machine_source()`) existe déjà. Corrigé de la même façon via `Oneshot::machine_numero_identification_wiring()` (fichier + BDD, idempotent, exécuté et vérifié sur gvv.net).
- [x] Extension de `forms.required_params` à la dimension machine (limite identifiée ci-dessus, corrigée) : un formulaire GVV peut dépendre d'un pilote, d'un instructeur et/ou d'une machine. Migration 174 étend l'ENUM aux 8 combinaisons (`none`/`pilot`/`instructor`/`machine`/`pilot+instructor`/`pilot+machine`/`instructor+machine`/`pilot+instructor+machine`), appliquée sur gvv.net. Nouveau helper `application/helpers/forms_params_helper.php` (`forms_requires_pilot()`/`forms_requires_instructor()`/`forms_requires_machine()`, autoloadé) pour ne pas dupliquer la liste des combinaisons dans le contrôleur et les vues. `Forms_admin::generate()`/`generate_submit()` gèrent désormais un sélecteur machine (`_get_machine_selector()`, table `machinesa` uniquement — seule table que `machine.*` sait résoudre) et transmettent `machine_immat` dans l'URL générée, avec validation si requis. Vue `bs_generate.php` : bloc sélecteur machine conditionnel ; `bs_form.php` : 4 nouvelles options dans le sélecteur "Contexte GVV" ; `bs_submissions.php` : condition simplifiée en `required_params !== 'none'`. `forms.required_params` des 2 formulaires ULM passé à `pilot+instructor+machine` via `Oneshot::forms_required_params_add_machine()` (idempotent, exécuté sur gvv.net). Langues FR/EN/NL mises à jour. Test PHPUnit `application/tests/mysql/FormsAdminGenerateMachineTest.php` (3 tests : erreur si machine manquante, `machine_immat` transmis dans l'URL, pré-remplissage bout-en-bout sur le formulaire public) — suite `Forms` complète rejouée sans régression (65/65) ; suite complète du projet rejouée sans régression (1961 tests, 63 skips pré-existants). Vérifié manuellement de bout en bout sur gvv.net (génération → URL → rendu pré-rempli avec la vraie valeur `machinesa.numero_identification`).
