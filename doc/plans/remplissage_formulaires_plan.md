...
- [x] Vérifier l'ouverture statique d'un formulaire exporté dans un navigateur nu (sans serveur), CSS résolu. `Forms_file_storage::write_page()` enveloppe désormais chaque page dans un document HTML5 autonome (`<!DOCTYPE>/<html>/<head><link rel="stylesheet" href="style.css"></head><body>...`) écrit directement sur disque ; `read_page()` désenveloppe symétriquement pour que le reste de l'application (rendu public, parsing des champs, PDF, `content_html` en base) continue de manipuler un simple fragment, inchangé. Les 8 formulaires existants ont été réenveloppés. **Reste ouvert** : l'archive ZIP exportée (`form_backup()`) source encore `pages/NN.html` depuis `content_html` en base (fragment nu, pas encore enveloppé) — traité au Lot 2-ter.
...
- [ ] Introduire `meta.json` dans `uploads/formulaires/{code}/` (titre, description, css_scope, paramètres requis, options d'export, liste des pages avec titres) ; écrit à chaque `store()`/`update()` du formulaire, pas seulement à l'export.
- [ ] Aligner l'archive ZIP sur le répertoire réel : réutiliser `wrap_page_html()`/`unwrap_page_html()` (déjà utilisées par `write_page()`/`read_page()`) pour que `form_backup()`/`form_import_zip()`/`form_restore()` manipulent les mêmes fichiers enveloppés que le stockage courant, plus `meta.json` à la racine de l'archive — suppression du format d'archive spécifique actuel (`pages/NN.html` en sous-dossier, fragments nus).
- [ ] Retirer les zones de saisie libre HTML/CSS de l'admin (`bs_form.php`, `bs_page_form.php` : textarea `content_html`, textarea `global_css`, mode texte de `page_import()`) ; conserver uniquement `form_import_html()` (dépôt d'un fichier HTML unique, page 1 d'un nouveau formulaire) et le dépôt d'archive (création et remplacement via `form_restore()`).
- [ ] Faire de `form_restore()` le chemin de modification normal (pas seulement une opération de restauration de secours) : vérifier que le message/CTA de l'interface reflète cet usage courant, sans changer la garantie déjà en place (code/statut/lien public non modifiés par une restauration).
- [ ] CSS partagé : réserver `uploads/formulaires/.commun/style.css` (nom hors alphabet `alpha_dash` des codes de formulaire, donc sans collision possible), ajouter une route publique dédiée pour le servir (même principe que `forms_public/image/{code}/{filename}`), documenter la convention `@import url("https://.../forms_public/shared_css")` en tête d'un `style.css` de formulaire.
- [ ] Adapter le contrôleur (`forms_admin.php`) : la table `forms` ne reste que l'index (identifiant stable, statut, section, slug, timestamps/audit) — les champs de contenu/configuration désormais dans `meta.json` cessent d'être la référence en base (peuvent y rester en cache best-effort, comme `content_html`/`global_css` aujourd'hui).
- [ ] Test PHPUnit : lecture/écriture de `meta.json`, symétrie archive ↔ répertoire (export puis import reproduit le même contenu), route de CSS partagé, non-régression `form_restore()` (code/statut/slug préservés).
- [ ] Test Playwright smoke : création d'un formulaire par dépôt d'archive (plus de formulaire de création par champs HTML/CSS), modification de contenu par `form_restore()`, rendu public avec CSS partagé chargé.
- [ ] Documentation utilisateur (`doc/users/fr/13_formulaires.md`) : remplacer les instructions de collage HTML/CSS par le flux d'archive ; documenter le CSS partagé. Mettre à jour le [tutoriel IA](../users/fr/13_formulaires_tutoriel.md), dont le flux actuel de copier-coller par page devient un flux de dépôt d'archive unique.
