...
## Objectifs
- Permettre l’archivage de documents associés aux pilotes et/ou au club/sections.
- Gérer des documents avec ou sans date de validité.
- Conserver l’historique des versions lors du remplacement d’un document.
- Mettre en évidence les documents expirés.
- Mettre en place un workflow de validation par administrateur.
- Donner aux administrateurs un accès direct aux documents à valider et aux documents expirés.
- Permettre aux utilisateurs de s’abonner à des alertes d’expiration par email.
- Permettre aux utilisateurs d'être notifiés avant l'expiration des documents lors de leur connexion.
- Le mécanisme pourra être utilisé comme stockage par le module d'acceptation de documents qui sera détaillé dans un autre PRD. Le but sera alors d'enregistrer les acceptations de documents légaux (charte, RGPD, etc.) par les membres.
...
## Portée
### Inclus
- Documents liés à un pilote (ex. visite médicale) et documents non associés à un pilote (ex. documents club).
...
- Affichage d’indicateurs visuels pour documents expirés.
- Alertes email avant expiration.

### Exclu
- Modification automatique des statuts de membres ou de droits à partir des documents.
...
- Qui peut s’abonner aux alertes (pilotes uniquement, administrateurs, tous les membres) ? Ca dépend du type de document
...
- Faut-il distinguer des niveaux de confidentialité par type de document ? Plus tard peut-être
