/**
 * Asterix Recursive Authorization Test
 *
 * Logs in as asterix (simple user, role 'user', sections Planeur+Général)
 * and recursively crawls all internal links reachable from menus and
 * dashboards, verifying that every page is accessible 
 * 
 * When an access is forbiden it must be filtered out from the menu and dashboard. It
 * does not make sense to present an option to a user, and then to tell him that this choice is forbidden.
 *
 ...
 *
 * Usage:
 *   npx playwright test tests/asterix-recursive-authorizations.spec.js --reporter=line
 */
...
test.describe('Asterix Recursive Authorization Crawl', () => {
    ...
    test('all visible links must be accessible (forbidden URLs must not appear in menus)', async ({ page }) => {
        ...
        // --- Login (same pattern as asterix-authorization.spec.js) ---
