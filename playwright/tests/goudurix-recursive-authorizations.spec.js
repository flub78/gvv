/**
 * Goudurix Recursive Authorization Test
 *
 * Logs in as goudurix (user in Avion + Général, tresorier via mniveaux)
 * and recursively crawls all internal links reachable from menus and
 * dashboards, verifying that every page is accessible.
 *
 ...
 *
 * Usage:
 *   npx playwright test tests/goudurix-recursive-authorizations.spec.js --reporter=line
 */
...
test.describe('Goudurix Recursive Authorization Crawl', () => {
    ...
    test('all visible links must be accessible (forbidden URLs must not appear in menus)', async ({ page }) => {
        ...
        await loginPage.login(GOUDURIX.username, GOUDURIX.password, GOUDURIX.section);
        ...
        console.log(`[${GOUDURIX.username} / section ${GOUDURIX.section}] Starting crawl from ${startUrl}`);
        console.log(`[${GOUDURIX.username} / section ${GOUDURIX.section}] Initial queue: ${queue.length} unique route patterns`);
        ...
        while (queue.length > 0) {
            ...
            console.log(`  [${GOUDURIX.username} / section ${GOUDURIX.section}] Testing: ${url}`);
            ...
            try {
                ...
                if (isAccessDenied(finalUrl, content)) {
                    ...
                    console.log(`    => DENIED  [${GOUDURIX.username} / section ${GOUDURIX.section}]`);
                    continue;
                }

                // Access granted
                accessGranted.push({ url, pattern });
                console.log(`    => OK  [${GOUDURIX.username} / section ${GOUDURIX.section}]`);
        ...
        expect(accessDenied.length,
            `[${GOUDURIX.username} / section ${GOUDURIX.section}] ${accessDenied.length} visible link(s) returned access denied. ` +
        ...
        expect(errors.length,
            `[${GOUDURIX.username} / section ${GOUDURIX.section}] ${errors.length} route(s) had navigation errors:\n` +
