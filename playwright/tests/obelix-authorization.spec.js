/**
 * Obelix Authorization Tests - New Authorization System
 *
 * Tests that the new code-based authorization system (v2.0) correctly
 * manages access for obelix, a user with roles:
 *   - planchiste in section Planeur (1)
 *   - auto_planchiste in section ULM (2)
 *   - user in section Général (4)
 *
 * Obelix is enrolled in use_new_authorization, so require_roles() is active.
 *
 * Usage:
 *   npx playwright test tests/obelix-authorization.spec.js --reporter=line
 */

const { test, expect } = require('@playwright/test');
const LoginPage = require('./helpers/LoginPage');

const OBELIX = { username: 'obelix', password: 'password', section: '1' };

/**
 * Helper: login as obelix and navigate to a route
 */
async function loginAndGoto(page, route, section) {
    const loginPage = new LoginPage(page);
    await loginPage.open();
    await loginPage.login(OBELIX.username, OBELIX.password, section || OBELIX.section);
...
test.describe('Obelix Authorization - New Auth System', () => {

    // ============================================================
    // ALLOWED ROUTES - obelix has 'user' role (all sections)
    // ============================================================
    ...
    // ============================================================
    // DENIED ROUTES - planchiste stats (requires more than planchiste)
    // ============================================================
