/**
 * Obelix Recursive Authorization Test
 *
 * Logs in as obelix (planchiste in Planeur, auto_planchiste in ULM,
 * user in Général) and recursively crawls all internal links reachable
 * from menus and dashboards, verifying that every page is accessible.
 *
 ...
 *
 * Usage:
 *   npx playwright test tests/obelix-recursive-authorizations.spec.js --reporter=line
 */
...
test.describe('Obelix Recursive Authorization Crawl', () => {
    ...
    test('all visible links must be accessible (forbidden URLs must not appear in menus)', async ({ page }) => {
        ...
        await loginPage.login(OBELIX.username, OBELIX.password, OBELIX.section);
