/**
 * Panoramix Recursive Authorization Test
 *
 * Logs in as panoramix (club-admin in all sections) and recursively
 * crawls all internal links reachable from menus and dashboards,
 * verifying that every visible page is accessible.
 *
 * When an access is forbidden it must be filtered out from menus and
 * dashboards. It does not make sense to present an option to a user,
 * and then to tell him that this choice is forbidden.
 *
...
const PANORAMIX = { username: 'panoramix', password: 'password', section: '1' };
...
test.describe('Panoramix Recursive Authorization Crawl', () => {
    ...
    test('all visible links must be accessible (forbidden URLs must not appear in menus)', async ({ page }) => {
        ...
        await loginPage.login(PANORAMIX.username, PANORAMIX.password, PANORAMIX.section);
        ...
        while (queue.length > 0) {
            ...
            console.log(`  [${PANORAMIX.username}] Testing: ${url}`);
            ...
            try {
                ...
                if (isAccessDenied(finalUrl, content)) {
                    ...
                    console.log(`    => DENIED  [${PANORAMIX.username} / section ${activeSection}]`);
                    continue;
                }

                // Access granted
                accessGranted.push({ url, pattern });
                console.log(`    => OK  [${PANORAMIX.username} / section ${activeSection}]`);
        ...
        expect(accessDenied.length,
            `[${PANORAMIX.username}] ${accessDenied.length} visible link(s) returned access denied. ` +
        ...
        expect(errors.length,
            `[${PANORAMIX.username}] ${errors.length} route(s) had navigation errors:\n` +
