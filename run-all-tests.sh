...
show_help() {
    ...
    echo ""
    echo "Run all GVV PHPUnit tests (~130 tests across all suites)"
    echo ""
    echo "Options:"
    ...
    echo "  - Unit Tests (30 tests): helpers, models, libraries, i18n"
    ...
    echo "  Total: ~138 tests"
    echo ""
    echo "──────────────────────────────────────────────────────────────────"
    echo "Test Suite Commands :"
    echo "──────────────────────────────────────────────────────────────────"
    echo ""
    echo "# Suite 1/5: Unit Tests"
    echo "phpunit --configuration phpunit.xml"
    echo "phpunit --configuration phpunit.xml application/tests/unit/helpers/BitfieldsHelperTest.php"
    echo ""
    echo "# Suite 2/5: URL Helper Tests"
    echo "phpunit --configuration phpunit_url_helper.xml"
    echo ""
    echo "# Suite 3/5: Integration Tests"
    echo "phpunit --configuration phpunit_integration.xml"
    echo ""
    echo "# Suite 4/5: Enhanced CI Tests"
    echo "phpunit --configuration phpunit_enhanced.xml"
    echo ""
    echo "# Suite 5/5: MySQL Tests"
    echo "phpunit --configuration phpunit_mysql.xml"
    echo "phpunit --configuration phpunit_mysql.xml application/tests/mysql/ConfigurationModelMySqlTest.php"
    echo "phpunit --configuration phpunit_mysql.xml application/tests/mysql/EcrituresModelSoldeCompteGestionMySqlTest.php"
    echo "phpunit --configuration phpunit_mysql.xml --filter testSoldeCompteGestionBasicExecution application/tests/mysql/EcrituresModelSoldeCompteGestionMySqlTest.php"
    echo ""
    exit 0
}
...
echo "  Test suites: ALL (5 suites, ~130 tests)"
echo ""
...
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
run_suite "Unit Tests" "phpunit.xml" "Helpers, Models, Libraries, i18n" "unit"

echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Test Suite 2/5: URL Helper Tests${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
run_suite "URL Helper Tests" "phpunit_url_helper.xml" "URL generation and validation" "url_helper"

echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Test Suite 3/5: Integration Tests${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
run_suite "Integration Tests" "phpunit_integration.xml" "Real database operations, metadata" "integration"

echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
echo -e "${BLUE}  Test Suite 4/5: Enhanced CI Tests${NC}"
echo -e "${BLUE}═══════════════════════════════════════════════════${NC}"
